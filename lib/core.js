...
export function convertNewJSON(json){
  return typeof js !== 'object' || js === null ? js :
    Array.isArray(js) ? 
      Seq(js).toKeyedSeq().mapEntries(switchId).toOrderedMap() :
      Immutable.Seq(js).map(convertNewJSON).toOrderedMap();
	mapEntries<KM, VM>(
}
...
export function switchId([k,v]){
	if(v.id){
		return [v.id, convertNewJSON(v).merge]
	}else{
		return [k, convertNewJSON(v)]
	}	
}

export function(object, map){
	/*
		if you're dealing with an object
		we need to somehow pass the
