...
export function wrapMapState(js, tree, globe = Map()){
	//const startTime = new Date().getTime();
	const nextState = mapState(js.toJS ? js.toJS() : js, tree, Map().asMutable());
...
export function createInitialGlobe(globes, mapObject){
	try{
		...
		return recursiveSetInitialObject(globes, mapObject);
		...
		if(mapObject.get('nextObject')){
			if(mapObject.get('thisObject')){
				...
				if(nextGlobe){
					...
					console.log('mergeWithGlobe', mergeWithGlobe)
					if(mergeWithGlobe){
						try{
