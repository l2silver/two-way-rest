...
export function checkReset(resetTableName, reducerState, args){
	if(resetTableName){
		const location = ['batchReset', args.get('reset'), args.get('tree').first()]
		if( !reducerState.hasIn(location) ){
			args.get('dispatch')(customAction(state=>state.setIn(location, true)))
		}
	}
}
...
export function coreGET(args, type){	
	const resetTableName = args.get('reset');
	const reducerState = args.get('getState')()[args.get('reducer')]
