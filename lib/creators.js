...
export function checkMulti(element){
	if(element.attributes['multiple']){
	    return element.options.reduce((list, option)=>{
	    	if(option.selected){
...
export function create(args){
	return (dispatch, getState)=>{
		function responsePromise(args){
			...
			.then((response)=>{
				return responseCreator(response, args, args=>args.update('outTree', outTree=>outTree.push(response.id)))
			})
		}
...
export function destroy(args){
	return (dispatch, getState)=>{
		function reponsePromise(args){
			return postRequestCreator(args, args=>del(args.get('path')))
			.then((response)=>{
				return responseCreator(response, args)
			})
		}
