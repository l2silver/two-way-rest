	...
	*/
	samePageAndTree: function(nextProps){
		...
		for (let prop of propKeys) {
			if(!ignoreProps[prop]){
				if( !is(this.props[prop], nextProps[prop] ) ){
					return false
	  			}
			}
		}
		/*
		if(this.props.tree && this.props.tree[0]  && this.props.tree[0] == 'userInfo'){
			console.log('States', is(Map(this.props.state), Map(nextProps.state)), this.tree().toJS(), Map(this.props.state).toJS(), Map(nextProps.state).toJS())
		}
		*/
		return is(this.tree(), this.tree(nextProps)) && is(Map(this.props.state), Map(nextProps.state))
	},
