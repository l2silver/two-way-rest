...
import {Seq, Map, List, is, OrderedMap, fromJS} from 'immutable';
...
export function generateTree(tree, location){
...
export function runBatchBatchDispatch(actions){
	store.dispatch(batchActions(actions))
}

export function getTree(tree){
	return List(tree.split('/'))
}

export function getTreeFromLocation(location){
	const fullUrl = List(location.split('/'))
	const urlSize = fullUrl.size
	const last = urlSize > 0 ? urlSize - 1 : 'false'
	const secondLast = urlSize > 1 ? urlSize - 2 : 'false'
	const thirdLast = urlSize  > 2 ? urlSize - 3 : 'false'
	const treeProperties = [last, secondLast, thirdLast].reduce((treeProperties, index)=>{
		if(index != 'false'){
			const possibleTreeComponent = fullUrl.get(index);
			if(possibleTreeComponent){
				if(		possibleTreeComponent == 'edit' 
					|| 	possibleTreeComponent == 'index' 
					|| 	possibleTreeComponent == 'show'  
					|| 	possibleTreeComponent == 'create'
					|| 	possibleTreeComponent == 'update'){
	...
	if(treeProperties.has('id') && treeProperties.has('name')){
		...
		return List([treeProperties.get('name'), treeProperties.get('id')])
	}
	if(treeProperties.has('name')){
	
		return List([treeProperties.get('name')])
	}
	return false
}
...
export const defaultProperties = Map({
	...
	unmount: function(){
		if(this.props.truncateTable){
			this.custom(
				state=>state.deleteIn(this.tree())
			);
		}
	},
	mount: function(){
	...
	sameGlobe: function(nextProps){
		...
		if(live){
			const  bool = live.toSeq().reduce((bool, table)=>{
				...
				if(bool){
					return this.context.getState()[reducer].get(tableName) == this.oldState[reducer].get(tableName)
				}
				return false
			}, true)
			if(!bool){
		...
		if(substate){
			return substate.toSeq().reduce((bool, table)=>{
				...
				const reducer = table.get('reducer')
				if(bool){
					return this.context.getState()[reducer].getIn(['Substate', tableName ]) == this.oldState[reducer].getIn(['Substate', tableName ])
				}
				return false
			}, true)
		}
	...
	componentWillMount: function(){
		...
		this.oldState = this.context.getState();
	},
	...
	instance: function(){
		if(!this.changedState || typeof this.getInstance != undefined){
			this.getInstance = this.generateInstance();
		}
