	...
	if(instance && instance.get && instance.get('errors')){
		...
		if(errors){
			return <div className='default-errors'>{
				typeof errors == 'object' ? mapIf(errors, (error)=>{
...
export const defaultProperties = Map({
	...
	*/
	shouldComponentUpdate: function(nextProps){
		return !this.checkForDifferentPageRerender(this.tree(), this.tree(nextProps))
	},
	...
	page: function(props){
		const useProps = this.whichProps(props);
		const State = useProps.state;
		if(State){
			const reducer = this.reducer();
			return State[reducer].set('_globeTWR', State[reducer]);
		}
		return false;
	},
