...
import {createArgs, defaultProperties, defaultGetProperties} from './componentProperties';
const i = inflect(true);

/*
export const StupidTWRUpdate = React.createClass(defaultProperties.merge(defaultPostUpdateProperties).toJS());

export const StupidTWXUpdate = React.createClass(defaultProperties.merge(defaultPostFrontProperties.merge).merge({
...
export const StupidTWRDestroy = React.createClass(defaultProperties.merge().merge({
	submitForm: function(event){
		event.preventDefault();
...
export const StupidTWRUpload = React.createClass(defaultProperties.merge(defaultPostProperties).merge({
	submitForm: function(event){
		event.preventDefault();
		this.props.upload(createArgs(this, event.target));
	}
}).toJS())

export const StupidTWRCreate = React.createClass(defaultProperties.merge(defaultPostSubstateProperties).merge({
	tree: function(){
		return List(this.props.tree).unshift('Substate').push(this.getId())
	}
}).toJS());

export const StupidTWRCreateChild = React.createClass(defaultProperties.merge(defaultPostSubstateProperties).merge({
	tree: function(){
...
export const TWXUpdate = connect(
	mapStateToProps
	, actionCreators
)(StupidTWXUpdate);
