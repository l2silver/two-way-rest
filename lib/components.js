...
export const StupidTWRXUpdate = React.createClass(
	...
	.merge({
		...
		tree: function(){
			return List(this.props.instance.get('tree'));
		},
		outTree: function(){
...
export const StupidTWRDestroy = React.createClass(
	...
	submitForm: function(event){
		...
		if(this.props.noPrompt){
			return this.props.destroy(createArgs(this, findDOMNode(this)));
		}
...
export const StupidTWRDestroyFront = React.createClass(
	...
	submitForm: function(event){
		event.preventDefault();
		if(this.props.noPrompt){
			return this.props.destroyFront(createArgs(this, findDOMNode(this)));
		}
		const result = confirm(this.props.prompt ? this.props.promp : 'Are you sure you want to delete?');
		if(result){
			return this.props.destroyFront(createArgs(this, findDOMNode(this)));
...
export const StupidTWRIndexFront = React.createClass(defaultProperties.merge(defaultGetRenderProperties).toJS());
...
export const StupidTWRCreateFront = React.createClass(defaultProperties.merge(defaultPostCreateProperties).merge({
	...
	submitForm: function(event){
		event.preventDefault();
		const tree = this.instance().get('tree');
		try{
			const dom = findDOMNode(this)
			this.props.createFront(createArgs(this, dom).update(
			'content'
			, content=>content.set(
				'id'
				, this.getId()
			...
			console.log('A horrible error', e);
		}
	}
}).toJS());
...
export const StupidTWRCreateChildFront = React.createClass(defaultProperties.merge(defaultPostCreateProperties).merge({
	...
	submitForm: function(event){
		...
		const tree = this.props.instance.get('tree');
		const parentInstanceName = tree.pop().last();
		this.props.createFront(createArgs(this, findDOMNode(this)).update(
			'content'
			, content=>content.set(
				'id'
				, this.getId()
