...
export function getElementContent(element, nextObject){
...
export function getContent(content, previousContent = Map()){
	const childNodes = content.childNodes ? List(content.childNodes) : [];
	return childNodes.reduce((nextObject,element)=>{
...
export function postRequestCreator(args, fn){
	if(args.get('upload')){
		...
		return up(args.get('path'), formData, args.get('twr') );
	}
	return fn(args);
}
